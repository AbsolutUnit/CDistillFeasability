#!/home/anook/anaconda3/envs/cs7643-finalproj/bin/env python
# coding=utf-8
...
from options import DataTrainingArguments, ModelArguments, WandbArguments, FtArguments, ContextDistillArguments
...
from models.opt_wrapper import OPTWithClassifier, OPTWithLMClassifier, OPTWithLMClassifierCD
...
def main():
    ...
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments, TrainingArguments, WandbArguments, FtArguments, ContextDistillArguments))
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script and it's the path to a json file,
        # let's parse it to get our arguments.
        model_args, data_args, training_args, wandb_args, ft_args, cd_args = parser.parse_json_file(
            json_file=os.path.abspath(sys.argv[1]))
    else:
        model_args, data_args, training_args, wandb_args, ft_args, cd_args = parser.parse_args_into_dataclasses()
    ...
    if "gpt-j" in model_args.model_name_or_path:
        ...
        """
        If using Context Distillation, use OPTWithLMClassifierCD
        and make copy of model, using p_zero as context model, frozen params
        """
        if ft_args.target_tokens is not None and cd_args.context_distill:
            model = OPTWithLMClassifierCD.from_pretrained(
                model_args.model_name_or_path,
                from_tf=bool(".ckpt" in model_args.model_name_or_path),
                config=config,
                cache_dir=model_args.cache_dir,
            ...
            # Copy model to preserve the original weights and freeze current
            if training_args.do_train:
                model.p_zero = copy.deepcopy(model)

                for param in model.p_zero.parameters():
                    param.requires_grad = False

        elif ft_args.target_tokens is not None:
    ...
    max_context_len = min(
        cd_args.max_context_len, tokenizer.model_max_length)

    def dataset_context_reformat(context_targets):
        """
        For Context Distillation, the few shot context needed is gathered
        This is via using the target context tokens and their mapping
        which is used by helper method create_few_shot_context.
        We then select the few_shot that are in the selected range from the helper
        with added conditional that they are within the raw_train dataset size
        and then set the training dataset to that since we don't want to train on eval data
        """
        processed_target_tokens = [x.strip() for x in context_targets]
        processed_target_ids = {}
        
        for i in range(len(processed_target_tokens)):
            processed_target_ids[i] = processed_target_tokens[i]
        
        few_shot, few_shot_idx = create_few_shot_context(
            data_args.task_name,
            raw_datasets["train"],
            cd_args.num_shots,
            cd_args.input_pattern,
            processed_target_ids,
            cd_args.separate_shots_by,
            cd_args.description,
            cd_args.target_prefix,  
            cd_args.shot_idx,
            cd_args.balanced,
            cd_args.shuffle,
            training_args.seed
        ...
        selection = []
        for x in raw_datasets['train']['idx']:
            if x not in few_shot_idx and x < len(raw_datasets["train"]):
                selection.append(x)

        raw_datasets["train"] = raw_datasets["train"].select(selection)

        return few_shot, few_shot_idx

    if cd_args.context_distill:
        few_shot, few_shot_idx = dataset_context_reformat(cd_args.context_targets)

    def preprocess_function(examples):
        # Tokenize the texts
        """
        The additional conditionals for cd_args.context_distill check for Distillation
        If so, then we need to grab samples from the few shot dataset aligning with the pattern
        and then grab examples from what's passed in, casted to that format.
        This is then passed into a tokenizer with max_length being set to the extra context max length.

        We also set the base_values to context results since they are CD specific.
        """
        if cd_args.context_distill:
            match = f"{few_shot}{cd_args.input_pattern}" if few_shot else cd_args.input_pattern
            match = f"{match} {cd_args.target_prefix.strip()}" if cd_args.target_prefix else match

            samples = [
                match.format(
                    text1 = examples[sentence1_key][i],
                    text2 = examples[sentence2_key][i] if sentence2_key else None)
                for i in range(len(examples[sentence1_key]))
            ]

            args = (samples,)
            context_result = tokenizer(*args, padding = padding, max_length = max_context_len, truncation = True)
        ...
        if cd_args.context_distill:
            result["base_input_id"] = context_result["input_ids"]
            result["base_attention_mask"] = context_result["attention_mask"]
