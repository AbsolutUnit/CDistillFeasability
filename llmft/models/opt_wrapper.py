...
class OPTWithClassifier(OPTForSequenceClassification):
    def __init__(self, config):
        print("Using OPTWithClassifier")
        print(f"Opt classifier module: {self.named_parameters()}")
        super().__init__(config)
...
"""
Our implementation of OPT with LM 
Which enables Context Distillation
"""
class OPTWithLMClassifierCD(OPTWithLMClassifier):
    """
    OPTWithLMClassifier with custom Context Distillation support
    Based on "A General Language Assistant as a Laboratory for Alignment" [https://arxiv.org/pdf/2112.00861]
    """

    def __init__(self, configuration):
        """
        Start by calling super constructor
        And creating variable for p_zero model
        """
    ...
    def forward(
        ...
        input_ids: Optional[torch.LongTensor] = None,
        attention_mask: Optional[torch.FloatTensor] = None,
        base_input_id: Optional[torch.LongTensor] = None,
        ...
        Takes in the attn_out, hidden_out, and dict_return flags and parses/defaults to model
        Calls decoder of model, which gives logits, hidden states, and attentions
        Extract logits and calculate sequence length via non-padding, seleting last non-padding logits
        Calculates KL Div loss between logits and context from p_zero
        If no base_input is given, loss is calculated as CE between logits and labels
        """
        ...
        sequence_len = torch.ne(input_ids, self.config.pad_token_id).sum(-1) - 1
        logits = logits[torch.arange(input_ids.shape[0], device=logits.device), sequence_len]

        loss = None
        if labels is not None:
            if base_input_id is not None:
                # Sample probabilities for p_zero
                p_zero_log_probs = F.log_softmax(self.p_zero.lm_head(
                    self.p_zero.model.decoder(
                        input_ids=base_input_id,
                        attention_mask=base_attention_mask)
                        [0]), dim=1)
                # Take the top 50
                p_zero_top50 = torch.topk(p_zero_log_probs, 50, dim=1)
                # Calculate the theta probabilities
                p_theta_log_probs = F.log_softmax(logits, dim=1)

                p_theta_top50 = torch.stack(
                    [p_theta_log_probs[idx][p_zero_top50.indices[idx]] for idx in range(len(logits))]
                )

                loss = nn.KLDivLoss(reduction="batchmean", log_target=True)(p_theta_top50, p_zero_top50.values)
            else:
                logits = logits.contiguous()
                labels = labels.to(logits.device)
                labels = labels.contiguous()
